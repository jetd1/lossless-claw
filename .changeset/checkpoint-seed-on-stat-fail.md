...
This restores the contract that "permissive return ⟹ checkpoint exists" without re-introducing the unconditional refresh `#649` deliberately removed. The placeholder is written via `summaryStore.upsertConversationBootstrapState` directly so it does not depend on stat success. Subsequent turns either fast-path-recover from offset=0 once the transcript becomes statable (and idempotently ingest via `identity_hash` dedup), or get overwritten by a normal `refreshBootstrapState` call later in the same `afterTurn`.
